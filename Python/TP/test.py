...
cv2.createTrackbar(trackbar2_name, window_name, 200, slider2_max, on_trackbar_change)
cv2.createTrackbar(trackbar3_name, window_name, 8000, 10000, on_trackbar_change)
...
def Tp():
    ...
    while True:
        ...
        contDnis1, _ = cv2.findContours(closing1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # sin este if se rompe el programa cuando el trackbar esta en 0
        if (threshVal_trackbar and maxThresh_trackbar) == 0:
            ...
            if filtered != []:
                ...
                cv2.drawContours(frame, [cnt], -1, (0, 255, 255), 12, cv2.LINE_AA)
